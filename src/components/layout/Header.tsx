...
const Header = (props: Props) => {
  ...
  return (
    <nav
      id="header"
      className={`fixed top-0 z-30 w-full text-white ${headerStyle.header}`}
    >
          ...
          <ul className="list-reset flex-1 items-center justify-end lg:flex">
              ...
              <Link
                ...
                className={`inline-block px-4 py-2 text-black no-underline ${
                  active
                    ...
                    : "hover:text-underline hover:text-gray-800"
                }`}
              >
                About
              </Link>
            ...
            <li className="mr-3">
              <Link legacyBehavior href={getRouteById("Home").path}>
                <button
                  id="navAction"
                  className={`mx-auto mt-4 rounded-full px-8 py-4 font-bold opacity-75 shadow hover:underline lg:mx-0 lg:mt-0 ${headerStyle.navaction}`}
                >
                  {user ? "App" : "Login"}
                </button>
              </Link>
