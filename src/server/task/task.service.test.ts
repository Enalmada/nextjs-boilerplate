...
describe("TaskService", () => {
	...
	describe("tasks", () => {
		...
		it("should throw NotAuthorizedError for member", async () => {
			const service = new TaskService();
			await expect(service.list(undefined, mockCtx)).rejects.toThrow(
				NotAuthorizedError,
			);
		});
	...
	describe("task", () => {
		it("should throw NotAuthorizedError for unauthorized task read", async () => {
			const service = new TaskService();
			await expect(service.get(mockTaskId, mockWrongCtx)).rejects.toThrow(
				NotAuthorizedError,
			);
		});
	...
	describe("update", () => {
		it("should throw NotAuthorizedError for unauthorized task update", async () => {
			const service = new TaskService();
			await expect(
				service.update(mockTaskId, mockTask, mockWrongCtx),
		...
		it("should throw OptimisticLockError for dirty update", async () => {
			...
			await expect(service.update(mockTaskId, input, mockCtx)).rejects.toThrow(
				OptimisticLockError,
			);
		});
