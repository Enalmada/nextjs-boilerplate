...
> {
	...
	protected constructor(
		...
		this.repository = createRepo<typeof schema, TEntity, TInput>(
			db,
			this.table,
			this.queryBuilder,
		);
	...
	async list(
		input: ListInput<TEntity> | undefined,
		ctx: MyContextType,
	): Promise<Page<TEntity>> {
		const logger = this.logger.logMethodStart(
			`${this.entityName} ${this.list.name}`,
			ctx,
			{
				...input,
			},
	...
	async create(input: TInput, ctx: MyContextType): Promise<TEntity> {
		const logger = this.logger.logMethodStart(
			`${this.entityName} ${this.create.name}`,
			ctx,
			{
				data: { ...input },
			},
	...
	async update(
		...
		const logger = this.logger.logMethodStart(
			`${this.entityName} ${this.update.name}`,
			ctx,
			{
				data: { id, ...input },
			},
	...
	async delete(id: string, ctx: MyContextType): Promise<TEntity> {
		const logger = this.logger.logMethodStart(
			`${this.entityName} ${this.delete.name}`,
			ctx,
			{
				id,
			},
