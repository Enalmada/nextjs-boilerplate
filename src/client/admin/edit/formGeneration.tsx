...
import { type GenericSchema, nullish, object, required, string } from "valibot";

export interface FormFieldConfig {
	key: string;
	label?: string;
	component?: "input" | "textarea" | "radio" | "date" | "hidden" | "radiobool";
	validation?: GenericSchema | null;
...
export const generateFormSchema = (config: FormFieldConfig[]) => {
	const schemaFields = config.reduce<Record<string, GenericSchema>>(
		(acc, field) => {
			if (field.validation || field.validation === undefined) {
				acc[field.key] =
					field.validation === undefined ? nullish(string()) : field.validation;
			}
			return acc;
		},
		{},
...
const FormFields = <T extends FieldValues>({
	...
	return (
		<div className={containerClass}>
			{config.map((field) => {
				const label =
					field.label === undefined ? capitalCase(field.key) : field.label;
