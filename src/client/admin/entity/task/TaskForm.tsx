...
import {
	...
	ADMIN_TASK,
	ADMIN_UPDATE_TASK,
} from "@/client/gql/admin-queries.gql";
import {
...
const nullishDateStringValidator = pipe(
	nullish(union([string(), date()])), // Allow nullish string or date inputs
	transform((input) => {
		if (input == null) {
			return null; // Handle null or undefined explicitly
		}
		return new Date(input); // Safely transform string/number/Date to Date
	}),
);
...
export default function TaskForm(props: Props) {
	const inputConfig: FormFieldConfig[] = [
		...
		},
		{
			key: "title",
			component: "input",
			validation: pipe(string(), minLength(1, "Title is a required field")),
		},
		{
			key: "description",
			component: "textarea",
