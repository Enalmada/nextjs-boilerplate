...
export async function testPageNavigation(
	locator: string,
	page: Page,
	config: PageTestConfig,
) {
	// Directly target links within the footer that contain the text
	const footerLinkLocator = page.getByRole("link", {
		name: config.textToClick,
	});
	...
	await expect(page.locator(locator)).toContainText(
		new RegExp(config.expectedText),
	);
}
